...
async def join_reqs(client, message: ChatJoinRequest):
    ...
    # 💯 Naya: Turant Approve karo taaki user Member ban jaye
    try:
        await client.approve_chat_join_request(message.chat.id, message.from_user.id)
    except Exception as e:
        print(f"Auto-Approve Error: {e}")

# 💯 Naya: Jab user channel leave karega, toh usko DB se hata dega
@Client.on_chat_member_updated(create(is_auth_req_channel))
async def handle_leave_member(client, member: ChatMemberUpdated):
    if member.new_chat_member.status in [enums.ChatMemberStatus.LEFT, enums.ChatMemberStatus.BANNED]:
        await db.remove_join_req(member.from_user.id, member.chat.id)
