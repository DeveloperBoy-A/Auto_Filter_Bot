...
async def start(client, message):
    ...
    if len(m.command) == 2 and m.command[1].startswith(("notcopy", "sendall")):
        try:
            _, user_id, verify_id, file_id = m.command[1].split("_", 3)
            user_id = int(user_id)
            ...
            if await db.is_user_verified(user_id):
                key, num, msg = "third_time_verified", 3, script.THIRDT_VERIFY_COMPLETE_TEXT
            elif await db.user_verified(user_id):
                key, num, msg = "second_time_verified", 2, script.SECOND_VERIFY_COMPLETE_TEXT
            else:
                key, num, msg = "last_verified", 1, script.VERIFY_COMPLETE_TEXT

            await db.update_notcopy_user(user_id, {key: now})
            await db.update_verify_id_info(user_id, verify_id, {"verified": True})

            if m.command[1].startswith("sendall"):
                link = f"https://t.me/{temp.U_NAME}?start=allfiles_{grp_id}_{file_id}"
            else:
                link = f"https://t.me/{temp.U_NAME}?start=file_{grp_id}_{file_id}"

            btn = [[InlineKeyboardButton("✅ ɢᴇᴛ ꜰɪʟᴇ ✅", url=link)]]

            await client.send_message(
                settings["log"],
                script.VERIFIED_LOG_TEXT.format(
                    m.from_user.mention, user_id,
                    now.strftime("%d %B %Y"), num
                )
            )

            sent = await m.reply_photo(
                photo=VERIFY_IMG,
                caption=msg.format(m.from_user.mention, get_readable_time(TWO_VERIFY_GAP)),
    ...
    if message.chat.type in (enums.ChatType.GROUP, enums.ChatType.SUPERGROUP):
        btn = [
            [InlineKeyboardButton("❤️ Add Me ❤️", url=f"https://t.me/{temp.U_NAME}?startgroup=true")],
    ...
    if len(message.command) == 1:
        ...
        gtxt = (
            "ɢᴏᴏᴅ ᴍᴏʀɴɪɴɢ 🌞" if now < 12 else
            "ɢᴏᴏᴅ ᴀғᴛᴇʀɴᴏᴏɴ 🌓" if now < 17 else
            "ɢᴏᴏᴅ ᴇᴠᴇɴɪɴɢ 🌘" if now < 21 else
            "ɢᴏᴏᴅ ɴɪɢʜᴛ 🌑"
        )
    ...
    try:
        ...
        msg = await client.send_cached_media(
            ...
            file_id=file_id,
            caption=files.caption or files.file_name,
            cover=files.cover,
        ...
        await msg.delete()
        await k.edit_text("<b>File deleted successfully ✔️</b>")
