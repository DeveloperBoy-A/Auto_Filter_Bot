...
@Client.on_message((filters.regex(r"(@\w+\s+)?/(request|Request)") | filters.regex(r"#request") | filters.regex(r"#Request")) & filters.group)
async def requests(bot, message):
    ...
    if message.reply_to_message and SUPPORT_CHAT_ID == message.chat.id:
        ...
        try:
            if REQST_CHANNEL is not None:
                ...
                if len(content) < 3:
                    await message.reply_text("<b>ʏᴏᴜ ᴍᴜꜱᴛ ᴛʏᴘᴇ ᴀʙᴏᴜᴛ ʏᴏᴜʀ ʀᴇǫᴜᴇꜱᴛ [ᴍɪɴɪᴍᴜᴍ 3 ᴄʜᴀʀᴀᴄᴛᴇʀꜱ].</b>")
            if len(content) < 3:
                success = False
        except Exception as e:
        ...
        content = re.sub(rf"(@{bot_username}\s+)?/(request|Request)(@{bot_username})?|#(request|Request)", "", content, flags=re.IGNORECASE)
    ...
    if success:
        try:
            ...
            sent_msg = await message.reply_text("<b>ʏᴏᴜʀ ʀᴇǫᴜᴇꜱᴛ ʜᴀꜱ ʙᴇᴇɴ ᴀᴅᴅᴇᴅ! ᴘʟᴇᴀꜱᴇ ᴡᴀɪᴛ ꜰᴏʀ ꜱᴏᴍᴇ ᴛɪᴍᴇ.\n\nᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ ꜰɪʀꜱᴛ & ᴠɪᴇᴡ ʀᴇǫᴜᴇꜱᴛ.</b>", reply_markup=InlineKeyboardMarkup(btn))
