...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    size = get_size(files.file_size) if files.file_size else ""
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            f_caption = CUSTOM_FILE_CAPTION.format_map({
                "file_name": title,
                "file_size": size,
            ...
            logger.exception(e)
            f_caption = title  # safe fallback

    if not f_caption:
        f_caption = title

    await query.answer(
        url=f"https://telegram.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{files.file_id}"
    )
